...
*/
const $ = new Env("中青看点极速版看看赚");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""

let zqlookStartbody= $.isNode() ? (process.env.spzqlookStartbodybox ? process.env.spzqlookStartbodybox : "") : ($.getdata('spzqlookStartbodybox') ? $.getdata('spzqlookStartbodybox') : "")
...
if (!zqlookStartbody) {
 ...
 else if (zqlookStartbody.indexOf("\n") == -1) {
            zqlookStartbodyArr.push(zqlookStartbody)
 }
 else if (zqlookStartbody.indexOf("\n") > -1) {
            zqlookStartbodys = zqlookStartbody.split("\n")
 }
 else if (process.env.spzqlookStartbody && process.env.spzqlookStartbody.indexOf('\n') > -1) {
            zqlookStartbodyArr = process.env.spzqlookStartbody.split('\n');
            console.log(`您选择的是用"\n"隔开\n`)
 }
 else {
            zqlookStartbodys = [process.env.spzqlookStartbody]
 };
...
if ($request.url.match(/\/user.youth.cn\/v1\/Nameless\/getBoxReward/)) {
        ...
        if (zqlookStartbody) {
            if (zqlookStartbody.indexOf(bodyVal) > -1) {
                ...
                zqlookStartbodys = zqlookStartbody + "\n" + bodyVal;
                $.setdata(zqlookStartbodys, 'spzqlookStartbodybox');
                $.log(`${$.name}获取看看赚任务: 成功, spzqlookStartbodybox: ${bodyVal}`);
                // bodys = zqlookStartbodys.split("@")
                // $.msg($.name, "获取第" + bodys.length + "个看看赚任务请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'spzqlookStartbodybox');
            $.log(`${$.name}获取看看赚任务: 成功, spzqlookStartbodybox: ${bodyVal}`);
