...
let zq_cookie= $.isNode() ? (process.env.zq_cookie2 ? process.env.zq_cookie2 : "") : ($.getdata('zq_cookie2') ? $.getdata('zq_cookie2') : "")
let zq_cookieArr = []
let zq_cookies = ""
...
if (!zqlookStartbody2) {
     ...
     $.done()
            ...
            zqlookStartbodyArr.push(zqlookStartbody2)
 }
 else if (zqlookStartbody2.indexOf("&") > -1) {
            zqlookStartbodys = zqlookStartbody2.split("&")
 }
 else if (process.env.zqlookStartbody2 && process.env.zqlookStartbody2.indexOf('&') > -1) {
            zqlookStartbodyArr = process.env.zqlookStartbody2.split('&');
            console.log(`您选择的是用"&"隔开\n`)
 }
 else {
            zqlookStartbodys = [process.env.zqlookStartbody2]
 };
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.success === true ){
                    ...
                    console.log('\n激活看看赚任务失败')
                    smbody = $.getdata('zqlookStartbody2').replace(zqlookStartbody1 + "&", "");
