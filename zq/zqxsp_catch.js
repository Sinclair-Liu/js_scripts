...
function getzqwzbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/V5\/Article\/kyComplete.json/)) {
        ...
        if (zqwzbody) {
            if (zqwzbody.indexOf(bodyVal) > -1) {
                ...
                $.setdata(zqwzbodys, 'zqxspbody');
                $.log(`${$.name}获取小视频: 成功, zqwzbodys: ${bodyVal}`);
                bodys = zqwzbodys.split("&")
                // $.msg($.name, "获取第" + bodys.length + "个阅读请求: 成功🎉", ``)
            }
        } else {
            $.setdata(bodyVal, 'zqxspbody');
            $.log(`${$.name}获取小视频: 成功, zqwzbodys: ${bodyVal}`);
            $.msg($.name, `获取第一个小视频请求: 成功🎉`, ``)
        }
    }
