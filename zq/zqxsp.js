...
const $ = new Env("中青看点阅读小视频");
const notify = $.isNode() ? require('./sendNotify') : '';
message = ""


let zqwzbody= $.isNode() ? (process.env.zqxspbody ? process.env.zqxspbody : "") : ($.getdata('zqxspbody') ? $.getdata('zqxspbody') : "")
let zqwzbodyArr = []
let zqwzbodys = ""

let zq_timebody= $.isNode() ? (process.env.zq_timebody2 ? process.env.zq_timebody2 : "") : ($.getdata('zq_timebody2') ? $.getdata('zq_timebody2') : "")
let zq_timebodyArr = []
let zq_timebodys = ""
const jc_timeheader={
...
function wzjl(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/V5/Article/kyComplete.json',
            headers : wzheader,
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if(result.items.read_score !== "undefined" ){
                    console.log('\n浏览小视频成功，获得：'+result.items.read_score + '金币')
                }else{
                    console.log('\n看太久了，换一篇试试')
                }
            } catch (e) {
